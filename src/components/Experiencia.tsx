    ...
    <section id="experiencia" className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
      ...
      <div className="mt-8 max-w-3xl">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <h3 className="text-lg font-medium text-papel-quente">Desenvolvedora de Sistemas</h3>
          <span className="font-mono text-xs text-menta">jan/2025 – atual</span>
        </div>
        <p className="mt-1 text-sm text-papel-quente/70">
          Setor Financeiro, Cooperativa (SICOOB)
        </p>
        <p className="mt-4 text-papel-quente/85">
        ...
        <h4 className="mt-6 text-sm font-medium text-papel-quente">Principais entregas</h4>
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-papel-quente/85">
          {entregas.map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ul>

        <h4 className="mt-6 text-sm font-medium text-papel-quente">Áreas de atuação</h4>
